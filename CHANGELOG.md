...
- 操作日志改为本地持久化，支持在设置中配置保留天数，内置 `7 天 / 30 天 / 自定义天数`
- 新增文件变更日志，每次配置写盘都会记录 1 条差异日志，可查看文件 / Server / Location 三级变更详情
- 配置管理新增“保存后自动重载”开关，默认开启，并在应用重启后保留用户选择

### 🎨 界面优化

- 配置管理搜索改为模式化搜索，支持按端口、按 location、按全局关键字三种方式筛选
- 详情 / 编辑弹窗移除左侧常驻 location 导航，改为顶部紧凑搜索跳转条，释放更多横向编辑空间
- 操作日志页重构为“统一列表 + 右侧详情抽屉”，并新增“仅看文件变更”筛选

### 🔧 改进

- 保存配置时仅阻塞到校验通过与写盘成功，随后立即关闭弹窗并刷新列表
- 保存后的自动重载改为后台执行，结果通过全局消息机制回推，不再阻塞页面继续操作
- 项目分类下拉去重，`重载配置` 按钮移到 `重新加载` 旁边，筛选区布局同步压缩整理

### 🐛 修复

- 修复配置管理搜索不准确的问题：端口搜索改为精确命中，location 与全局关键字搜索按各自范围正确匹配
- 修复日志仅保留应用运行期内容的问题，补齐关闭应用后的持久化回读
- 修复文件变更日志回载时可能覆盖前端未落盘普通日志的竞态问题
