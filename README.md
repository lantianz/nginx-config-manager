...
- ✅ **配置管理**：自动加载配置、查看 / 编辑 / 新增 / 删除 / 临时停用 Server 块，并支持保存后自动后台重载
- ✅ **项目分类**：基于 `server` 首行注释识别分类，支持分类筛选
- ✅ **定位编辑**：集成 Monaco Editor，支持顶部 location 搜索、跳转与高亮定位
- ✅ **操作日志**：支持本地持久化、保留天数设置、文件变更差异详情查看
- ✅ **应用设置**：统一管理 Nginx 路径、配置文件路径、日志保留天数与自动重载偏好
...
- 支持按端口 / location / 全局关键字、启用状态、项目分类筛选 Server
- 支持新增、编辑、删除、临时停用 / 恢复启用 Server
- 详情 / 编辑弹窗改为更宽的单栏编辑区，顶部可搜索 location 并跳转定位
- 保存时仅阻塞到配置校验和写盘成功，后续自动重载在后台执行，不阻塞弹窗关闭
- 支持直接打开配置文件、格式化全文、重载配置

### 3. 日志与设置

- 日志页支持查看普通操作日志与文件变更日志，文件变更可在右侧抽屉查看文件 / Server / Location 三级差异
- 日志支持本地持久化，普通日志按设置的保留天数清理，文件变更日志保留最近 3 次写盘记录
- 设置页统一管理 Nginx 路径、配置路径、主题、日志保留天数与保存后自动重载开关
...
4. **进入详情或编辑**：在弹窗顶部通过 location 搜索直接跳转对应配置位置
5. **保存配置**：保存成功后弹窗会立即关闭，若开启“保存后自动重载”则后台继续重载 Nginx
6. **进入日志页**：按需切换“全部日志 / 仅看文件变更”，查看最近的配置差异详情
7. **进入运行与端口**：执行启动、重载、配置校验或端口释放操作
...
**Q: 配置文件修改后不生效？**
A: 默认会在保存成功后自动后台重载；如果你关闭了该开关，则需要手动点击"重载配置"或"重启"。

**Q: 操作日志会在重启应用后丢失吗？**
A: 不会。日志已支持本地持久化，普通日志按设置中的保留天数清理，文件变更日志会保留最近 3 次。
